...
import { CheckCircle2, Clock, Search, XCircle, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/_app/history")({
  head: () => ({ meta: [{ title: "Post history — SocialSync 2.0" }] }),
  component: History,
});
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Every post published, scheduled, failed, or drafted in your workspace.
        </p>
      </div>
          ...
          <div className="divide-y divide-border">
            {loading ? (
              ...
              <div className="py-16 text-center text-sm text-muted-foreground">
                No posts match your active filter.
              </div>
            ) : (
              filtered.map((p) => (
                <div key={p.id} className="flex flex-wrap items-center gap-4 p-4 hover:bg-background/20 transition-colors">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      ...
                      <span>
                        {p.published_at
                          ? `Published: ${p.published_at.replace("T", " · ").slice(0, 16)}`
                          : p.scheduled_for
                          ? `Scheduled: ${p.scheduled_for.replace("T", " · ").slice(0, 16)}`
                          : "Created recently"}
                      </span>
                    </div>
                  </div>
